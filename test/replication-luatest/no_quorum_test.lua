...
pgroup:set_before_each(function(g)
    ...
    replica = cluster:build_server({args = {},}, {alias = 'no_quorum'}, engine, 'replica_no_quorum.lua')
...
pgroup:set_before_test('test_replication_no_quorum', function(g)
    ...
    t.helpers.retrying({timeout = 20},
        function()
            ...
            t.assert(fio.path.exists(SOCKET_DIR .. '/ready_' .. master.alias))
        end
    )
    master:eval("return box.schema.user.grant('guest', 'read,write,execute', 'universe', nil, {if_not_exists = true})")
...
pgroup:add('test_replication_no_quorum', function()
    ...
    t.helpers.retrying({timeout = 20},
        function()
            ...
            t.assert(fio.path.exists(SOCKET_DIR .. '/ready_' .. replica.alias))
